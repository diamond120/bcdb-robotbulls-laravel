...
class LoginController extends Controller
{
    ...
    public function login_email(AuthRequest $request)
    {
        if(recaptcha()) {
            $this->checkReCaptcha($request->recaptcha);
        }
...
//            $user->contributed = $user->contributed + rand(-25, 68);
//            $user->tokenBalance = $user->contributed;
        }
    ...
    public function checkLoginState(AuthRequest $request)
    {
        ...
        if (Auth::check()) {
            ...
            if ($user->status == 'active') {
                ...
                Auth::logout();
                return redirect(route('register'))->with(['danger' => __('messages.login.inactive')]);
            }
        } else {
            return redirect(url('/register'));
            // return view('intro'); 
    ...
    public function logout()
    {
        ...
        return (!is_maintenance() ? redirect(route('register')) : redirect(route('admin.register')));
    }
    ...
    public function registered()
    {
        ...
        if ($last_url == '/register') {
            return view('auth.message', compact('lang'))->with($data);
        }
