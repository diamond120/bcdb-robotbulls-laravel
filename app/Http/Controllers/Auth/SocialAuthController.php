...
class SocialAuthController extends Controller
{
    ...
    public function callback($social)
    {
        try {
            ...
            return view('auth.social', compact('user', 'social', 'notice', 'lang'));
        } catch (\Exception $e) {
    ...
    public function register(Request $request)
    {
        ...
        if($createUser){
            ...
            $createUser->tokenBalance = "0.000000";
            $createUser->contributed = "0.000000";
            $createUser->equity = "0.000000";
            $createUser->tokenBalance_demo = "0.000000";
            $createUser->contributed_demo = "0.000000";
            $createUser->equity_demo = "0.000000";
            $createUser->base_currency = 'usd';
    ...
    {
        if(!isset($request->token)) return redirect()->route('register')->with(['warning' => 'Invalid or Expired 2FA reset verification code!']);

        $token = $request->token;
        $meta = UserMeta::where('email_token', $token)->first();

        if(!isset($meta->user)) return redirect()->route('register')->with(['warning' => 'Invalid 2FA reset verification code!']);
