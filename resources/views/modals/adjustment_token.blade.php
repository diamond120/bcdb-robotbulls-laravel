    ...
    <div class="modal-dialog modal-dialog-md modal-dialog-centered">
        <div class=" sd modal-content">
            ...
            <div class="popup-body popup-body-md">
                <h3 class="popup-title">{{$lang['approve_transaction_of']}} . {{$trnx->tnx_id }}</h3>
                ...
                <form action="javascript:void(0)" method="POST" autocomplete="off" id="adjust_token_cal">
                    ...
                    <div class="row">
                        <div class="col-12">
                             <p>{{$lang['user_requested_to_purchase']}} <strong>{{$trnx->total_tokens.' '.token_symbol()}}</strong> {{$lang['token_and_payment_amount']}} <strong>{{ to_num($trnx->amount, 'max').' '.strtoupper($trnx->currency) }}</strong>{{$lang['please_update_received_amount_accrodingly']}}</p>
                             <div class="gaps-1-5x"></div>  
                         </div>
                         <div class="col-sm-6">
                            <div class="input-item input-with-label w-sm-60">
                                <label class="input-item-label">{{$lang['received_amount']}} <strong class="h5 text- text-info text-uppercase">({{$trnx->currency}})</strong></label>
                                <input class="input-bordered" required="" id="receive_amount" type="number" value="{{ to_num($trnx->amount, 'max') }}" name="receive_amount">
                            </div>
                        </div>
                        <div class="col-sm-6">
                            <div class="input-item input-with-label w-sm-60">
                                <label class="input-item-label">{{$lang['token_to_adjust']}}</label>
                        ...
                        <div class="col-12">
                            <div class="input-item">
                                ...
                                <label for="token_amount_adjust">{{$lang['check_to_confirm_adjusted_token']}}</label>
                            </div>
                        </div>
                    </div>
                    <div class="gaps-1x"></div>
                    <button type="submit" class="btn btn-md btn-auto btn-primary tnx-action" data-type="approved" data-id="{{ $trnx->id }}"><em class="far fa-check-square"></em><span>{{$lang['approve']}}</span></button>
                </form>
            </div>
        </div>
    </div>
