...
class WalletController extends Controller
{
    ...
    public function index()
    {
        ...
        $today = new \DateTime();
        $tnx_time = new \DateTime('2021-10-31');
        $diffTime = abs($today->getTimestamp() - $tnx_time->getTimestamp());
        $diffDays = ceil($diffTime / 86400);            
        
        $current_price = $json_a[$diffDays][1][3];
        ...
        return view(
            'user.wallet',
            compact('stage', 'currencies', 'bonus', 'bonus_amount', 'price', 'token_prices', 'is_price_show', 'minimum', 'active_bonus', 'pm_currency', 'contribution', 'lang')
        );
    }
    /**
...
//                $user->save();
    ...
    {
        if ($request->get('tnx_id') || $request->get('wallet')) {
            $id = $request->get('tnx_id');
            $pay_token = $request->get('wallet');
            if($pay_token != null){
            ...
            if(!empty($id)){
                ...
                return redirect(route('user.wallet'))->with(['danger'=>"Sorry, we're unable to proceed the transaction. This transaction may deleted. Please contact with administrator.", 'modal'=>'danger']);
            }
            if($tnx){
        ...
        return redirect(route('user.wallet'))->with(['danger'=>$name, 'modal'=>'danger']);
    }
}
