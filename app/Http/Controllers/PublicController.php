...
use App\PayModule\Stripe\StripePay;
use Auth;
use QRCode;
use Cookie;
use IcoData;
...
class PublicController extends Controller
{
    ...
    public function threeDAuth(Request $request){
        $stripePay = new StripePay();
        $secretKey = PaymentMethod::get_data('stripe')->clientSecret;
        $stripe = new Stripe\StripeClient($secretKey);
        $payment = $stripe->paymentIntents->retrieve($request->payment_intent,[]);
        $state = $payment->status == 'succeeded' ? 'approved' : 'pending';
        return $stripePay->stripe_success($payment->id,$state,$payment->amount,$payment->created,$payment);
    }
    /**
     * Show the QR Code
     *
            ...
            if (Auth::check()) {
                ...
                if ($user->email_verified_at != null && $user->status == 'active') {
                    if (is_2fa_lock()) {
                        ...
                        if ($user->role == 'admin') {
                            return view('admin.page', compact('page', 'lang'));
                        } else {
                            return view('user.page', compact('page', 'lang'));
                        }
                    }
                } else {
                    return view('public.page', compact('page', 'lang'));
