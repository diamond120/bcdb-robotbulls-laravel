...
class IcoHandler
{
    ...
    public static function _message()
    {
        if (empty(env_file()) || empty(app_key(1)) || empty(app_key(2)) || !nio_feature('cool') ) {
            $text = "<!--  v" . str_replace('.', '', config('app.version')).config('app.installed').config('app.update') . ". Application"."n"."io -->\n";
        } else {
    ...
    public static function get_manual_payment($type, $ext = '', $active = true)
    {
        ...
        if ($type == 'usd' || $type == 'eur' || $type == 'gbp' || $type == 'cad' || $type == 'aud' || $type == 'try' || $type == 'rub' || $type == 'inr' || $type == 'brl' || $type == 'nzd' || $type == 'pln' || $type == 'jpy' || $type == 'myr' || $type == 'idr' || $type == 'ngn' || $type == 'mxn' || $type == 'php' || $type == 'chf' || $type == 'thb' || $type == 'sgd' || $type == 'czk' || $type == 'nok' || $type == 'zar' || $type == 'sek' || $type == 'kes' || $type == 'nad' || $type == 'dkk' || $type == 'hkd' || $type == 'cdf' || $type == 'cfa') {
