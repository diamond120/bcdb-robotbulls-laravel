...
class Transaction extends BigChainModel
{
    ...
    public static function AdvancedFilter(Request $request) {
        ...
        if($request->filter){
            ...
            $trnxs = Transaction::whereNotIn('status', [$deleted, 'new'])->whereNotIn('tnx_type', ['withdraw'])
                        ->where(self::keys_in_filter( $request->only(['type', 'state', 'stg', 'pmg', 'pmc', 'duration', 'plan']) ))
    ...
    */
    protected static function keys_in_filter($request) {
        $result = [];
        $find = ['type', 'state', 'stg', 'pmg', 'pmc', 'duration', 'plan'];
        $replace = ['tnx_type', 'status', 'stage', 'payment_method', 'currency', 'duration', 'plan'];
    ...
    */
    ...
    protected static function duration_in_filter($request) {
        $get_duration = $request->duration;

        if($get_duration == '3_month'){
            $duration = "3 Month";
        }

        if($get_duration == '6_month'){
            $duration = "6 Month";
        }

        if($get_duration == '12_month'){
            $duration = "12 Month";
        }
    ...
    protected static function plan_in_filter($request) {
        $get_plan = $request->plan;

        if($get_plan == 'general_plan'){
            $plan = "General Bull";
        }

        if($get_plan == 'stocks_plan'){
            $plan = "Stocks Bull";
        }

        if($get_plan == 'crypto_bull'){
            $plan = "Crypto Bull";
        }

        if($get_plan == 'ecological_bull'){
            $plan = "Ecological Bull";
        }

        if($get_plan == 'nft_bull'){
            $plan = "NFT Bull";
        }

        if($get_plan == 'metaverse_bull'){
            $plan = "Metaverse Bull";
        }

        if($get_plan == 'ipo_bull'){
            $plan = "IPO Bull";
        }

        if($get_plan == 'real_estate_bull'){
    ...
    public static function dashboard($chart = 7) {
        ...
        $data['currency'] = (object) [
            ...
            'cdf' => round(self::amount_count('CDF')->total, $max),
            'cfa' => round(self::amount_count('CFA')->total, $max),
