...
function token_calc(e) {
    ...
    if (window.location.href.indexOf("invest") > -1) {v.val(O), g.val(D), address_btn(g.val(), minimum_token, maximum_token, O)}
    if (window.location.href.indexOf("contribute") > -1) {v.val(parseInt(I)), g.val(D), address_btn(g.val(), minimum_token, maximum_token, parseInt(I))}
    if (window.location.href.indexOf("test/contribute") > -1) {v.val(O*j), g.val(D), address_btn(g.val(), minimum_token, maximum_token, O*j)}
    console.log('O'+O)
    console.log('I'+I)
    console.log('z'+z)
    console.log('T'+T)
...
//    console.log("e:"+JSON.stringify(e));
        ...
        submitHandler: function (a) {
            $(a).ajaxSubmit({
                ...
                error: function (e, t, a) {
                    console.log("e:"+JSON.stringify(e)+" t:"+JSON.stringify(t)+" a:"+JSON.stringify(a));
    ...
    $(".token-payment-btn-btc-bull").click(function() {
        createCookie("amount", $("#token-number-btc-bull").val(), 10);
        console.log($("#token-number-btc-bull").val());
        createCookie("stripeToken", $("#token-number-btc-bull").val(), 10);
        console.log("token-number-btc-bull" + $("#token-number-btc-bull").val());
    ...
    $(".token-payment-btn-nft-bull").click(function() {
        createCookie("amount", $("#token-number-nft-bull").val(), 10);
        console.log($("#token-number-nft-bull").val());
        createCookie("stripeToken", $("#token-number-nft-bull").val(), 10);
        console.log("token-number-nft-bull" + $("#token-number-nft-bull").val());
    });
    ...
    e(document).on("click", ".payment-item", function (n) {
        ...
        if (e(this).find("label").attr("for") == "btc_bull") {
            plan = "BTC Bull";
            risk = $(".gb_risk").html();
            roi = $(".gb_roi").html();
            removeAllClasses();
        ...
        if (e(this).find("label").attr("for") == "nft_bull") {
            plan = "NFT Bull";
            risk = $(".nft_risk").html();
            roi = $(".nft_roi").html();
            removeAllClasses();
            $("#conclusion_name").addClass("badge-lighter");
