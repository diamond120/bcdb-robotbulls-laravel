...
class CoinpaymentsPay
{
    ...
    {
        $test="test";
        error_log( "test3 2 " . json_encode($test) . "\n", 3, "/home/robotbq/app_rb_folder/storage/logs/php.log");
...
//                'base_price' => $tc->calc_token($tokens, 'price')->base,
                ...
                'total_tokens' => '',
                'base_price' => '',
//                'amount' => round($tc->calc_token($tokens, 'price')->$currency/get_setting($pmc_auto_rate), max_decimal()),
                'amount' => $tokens/floatval(get_setting($pmc_auto_rate))*floatval(get_setting($pmc_crypto_rate)),
            ];
            $currency_rate = floatval(get_setting($pmc_crypto_rate));
            
            if ($request->input('pp_crypto') == "rbc") {
                $receive_currency = "rbc";
//                $base_currency = "rbc";
                $receive_currency = "rbc";
                $plan = "RBC";
                $duration = "";
                $percentage = "";
//                $amount_rbc = strval(1/ $currency_rate * $trnx_data['token']);
//                $amount_rbc = number_format($amount_rbc, 2, '.', '');
                $amount_rbc = $trnx_data['amount'];
    //            $amount_rbc = "1";
            } else {
...
//                $base_currency = $base_currency;
                ...
                $percentage = rand(123,355)/1000;
                $amount_rbc = $trnx_data['amount'];
            }
            
            $test="testg ";
            error_log( "test3GGG 2 " . json_encode($currency_rate) . " " .  json_encode(get_setting($pmc_auto_rate)) . "\n", 3, "/home/robotbq/app_rb_folder/storage/logs/php.log");
            error_log( "test3GGG 2 " . json_encode(gettype($currency_rate)) . " " .  json_encode(gettype(get_setting($pmc_auto_rate))) . "\n", 3, "/home/robotbq/app_rb_folder/storage/logs/php.log");
            error_log( "amount " . json_encode($trnx_data['amount']) . " " .  json_encode($trnx_data['token']) . "\n", 3, "/home/robotbq/app_rb_folder/storage/logs/php.log");
...
//                'total_tokens' => $trnx_data['total_tokens'],
                'total_tokens' => $trnx_data['token'],
                'stage' => active_stage()->id,
                'user' => Auth::id(),
                'amount' => $amount_rbc,
//                'base_amount' => $trnx_data['base_price'],
                'base_amount' => $trnx_data['token'],
                'base_currency' => $base_currency,
...
//                'all_currency_rate' => $all_currency_rate,
            ...
            try {
                ...
                $pcurrency = (get_pm('coinpayments', false)->sandbox == 1 ? $currency : $currency);
                $item_number = $trnx_data['token'];
                $item_name = $item_number." ". token_symbol()." Token Purchase";
                error_log( "before charge " . "\n", 3, "/home/robotbq/app_rb_folder/storage/logs/php.log");
                $charge = $this->cpApi()->CreateComplexTransaction($trnx_data['amount'], $pcurrency, $pcurrency, 'support@robotbulls.com', '', $auth_user->name, $item_name, strval($item_number), strval($tid), $custom, $ipn_url);
                error_log( "after charge " . "\n", 3, "/home/robotbq/app_rb_folder/storage/logs/php.log");
                error_log( "after chargeX " . json_encode($charge) . "\n", 3, "/home/robotbq/app_rb_folder/storage/logs/php.log");
                error_log( "after charge2 " . "\n", 3, "/home/robotbq/app_rb_folder/storage/logs/php.log");
                
            } catch(\Exception $e){} finally {
                if (!empty($charge) && $charge["error"] == "ok") {
                    error_log( "test3 2 catch " . "\n", 3, "/home/robotbq/app_rb_folder/storage/logs/php.log");
                    $return['msg'] = 'success';
                    ...
                    $_tnx->status = 'pending';
                    $_tnx->amount = $charge['result']['amount'];
                    $_tnx->plan = $plan;
                    $_tnx->duration = $duration;
                    $_tnx->percentage = $percentage;
                    $_tnx->save();
                    ...
                    error_log("test3 2 final error \n", 3, "/home/robotbq/app_rb_folder/storage/logs/php.log");
//                    error_log("test3 2 final error " . $e->getMessage() . " " . $e->getCode() . " " . $e->getFile() . " " . $e->getLine() . " " . $e->getTraceAsString() . "\n", 3, "/home/robotbq/app_rb_folder/storage/logs/php.log");
                    $return['msg'] = 'error';
    ...
    {
        $test="test";
        error_log( "test3 3 " . json_encode($test) . "\n", 3, "/home/robotbq/app_rb_folder/storage/logs/php.log");
        ...
        if ($chk && $chk->status != 'approved') {
            ...
            try {
                ...
                error_log( "test3 3 order" . json_encode($order) . "\n", 3, "/home/robotbq/app_rb_folder/storage/logs/php.log");
            } catch(\Exception $e){
            } finally {
                if ($order) {
                    ...
                    if ($order->status == 2) {
                        ...
                        if ($tnx !== null) {
                            ...
                            $tnx->receive_currency = ($order->status == 2) ? $order->coin : $user->base_currency;
...
//                        $this->send_cancel_email($tnx);
    ...
    {
        $test="test";
        error_log( "test3 4 " . json_encode($test) . "\n", 3, "/home/robotbq/app_rb_folder/storage/logs/php.log");
                        ...
                        if ($tnx !== null) {
                            ...
                            if ($order->status == 2) {
                                if ($_old_status == 'deleted') {
                                    $tnx->status = 'missing';
                                    $tnx->save();
                                    return redirect()->route('user.token')->with(['info', 'Thank you! We received your transfer but we found something wrong in your transaction, please contact with us with the order id: '.$tnx->tnx_id.'.']);
                                } else {
                                    if($tnx->status == 'approved' && is_active_referral_system()){
                                        $referral = new ReferralHelper($tnx);
                                        $referral->addToken('refer_to');
                                    ...
                                    return redirect(route('user.dashboard'))->with(['success'=>__('Your transfer was successful. Check your balance in your account.'), 'modal'=>'success']);
                                }
                            } else {
...
//                                    $this->send_cancel_email($tnx);
                                ...
                                return redirect(route('user.token'))->with(['warning'=> __('Sorry, we have not received your transfer!'), 'modal'=>'warning']);
                            }
                        }
                    }
                    if ($order->status < 0) {
                        ...
                        IcoStage::token_add_to_account($tnx, 'sub');
//                        $this->send_cancel_email($tnx);
                        return redirect(route('user.transactions'))->with(['warning'=>__('Sorry, we have not received your transfer!'), 'modal'=>'warning']);
                    }
                    if ($order->status == 0) {
                        return redirect(route('user.transactions'))->with(['warning'=>__('Your order is still pending, please make the transfer.'), 'modal'=>'danger']);
                    }
                } else {
                    return redirect(route('user.transactions'))->with(['danger'=>__('Sorry, the transaction was not found or  the order id is invalid.'), 'modal'=>'danger']);
                }
            }
        } else {
            if ($chk->status == 'approved') {
                return redirect(route('user.token'))->with(['success'=>__('Your transfer was successful. Check your balance in your account.'), 'modal'=>'success']);
            } else {
                return redirect(route('user.token'))->with(['warning'=>'Sorry, your order has been canceled! <br> And your payment Status is <strong>'. ucfirst($chk->status) .'</strong>', 'modal'=>'danger']);
            }
        }
    }
    ...
    {
        $test="test";
        error_log( "test3 5 " . json_encode($test) . "\n", 3, "/home/robotbq/app_rb_folder/storage/logs/php.log");
